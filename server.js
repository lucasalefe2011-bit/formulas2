...
Regras principais:
- Identifique a BASE DA FÓRMULA (em kg) diretamente do texto da ficha enviada.
- Essa base pode ser 30kg, 20kg, 25kg, 40kg, 200kg ou outro valor indicado.
- Se não houver base clara no texto, use 30kg como padrão.
- Escale proporcionalmente do peso base detectado até o peso alvo.
- Converta tudo para Y, considerando 1Y = 48 partes.
- Nunca arredonde os resultados, use sempre valores decimais exatos.
- Produto pode ser Textura, Grafiato, Projetada ou Selador.
...
Saída em JSON:
{
  ...
  const user = `Texto lido da ficha: ${rawText}
  ...
  try {
    ...
    if (ext === '.pdf') {
      ...
      const ocr = await openaiChat([
        ...
        { role: "user", content: [
          { type: "text", text: "Extraia todo o texto legível (PT-BR vírgula decimal)." },
